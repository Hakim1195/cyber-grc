...
const MesuresModule = (() => {
        ...
        const rows = mesures.map(m => {
            ...
            return `
                <tr class="clickable-row" data-id="${m.id}">
                    ...
                    <td class="t-centre">${Number(m.maturite) || 0}/5</td>
                    <td>${escapeHtml(m.responsable || "-")}</td>
                    <td class="t-centre">${cov.length === 0
    ...
    function actionsPanelHtml(m) {
        ...
        return `
            <div class="ref-actions-head">
                ...
                <button id="mesAddAction" class="txt-sm">Planifier une action</button>
            </div>
            ${list}
            <form id="mesActionForm" hidden style="margin-top:10px;">
                <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:flex-end;">
                    <div class="form-group" style="margin:0; flex:2 1 240px;"><label>Intitulé <span class="champ-requis" title="Champ obligatoire" aria-hidden="true">*</span></label><input id="mesActTitre" placeholder="Ex : Déployer le MFA sur tous les comptes à privilèges" /></div>
    ...
    function renderDetail(id) {
        ...
        const coverageHtml = cov.length === 0
            ? `<p class="txt-muted">Cette mesure ne couvre encore aucune exigence. Ouvrez le <strong>Détail</strong> d'une exigence dans un <a href="#/referentiels" class="lien-accent">référentiel</a> et reliez-la à cette mesure.</p>`
                ...
                <div style="margin-bottom:12px;">
                    ...
                    <ul class="ref-actions-list">
                        ${byRef[refId].items.map(c => {
                            ...
                            return `<li>
                                <a href="#/referentiels/${escapeHtml(c.refId)}" class="lien-accent">n°${escapeHtml(c.ev.code)} — ${escapeHtml(c.titre)}</a>
            ...
            <section class="page">
                <div class="dashboard-header">
                    <div>
                        ...
                        <p class="sous-titre"><a href="#/mesures" class="lien-accent">Mesures de sécurité</a> · pivot de conformité</p>
                    </div>
                ...
                <div class="dashboard-grid" style="grid-template-columns:2fr 1fr; align-items:start;">
                    <div class="dashboard-card">
                        <h3 class="mt0">Caractéristiques</h3>
                        <div class="form-group"><label>Nom <span class="champ-requis" title="Champ obligatoire" aria-hidden="true">*</span></label><input id="nom" value="${escapeHtml(m.nom)}" /></div>
                    ...
                    <div class="dashboard-card">
                        <h3 class="mt0">Propagation ${Help.tip("Recalcule le statut et la maturité des exigences couvertes, au plus défavorable de TOUTES leurs mesures (une exigence n'est conforme que si toutes ses mesures le sont). Cœur du « zéro double saisie ».")}</h3>
                        <p class="txt-muted-sm">Recalcule les <strong>${cov.length}</strong> exigence(s) couverte(s) « au plus défavorable » de leurs mesures (une exigence peut être couverte par plusieurs mesures).</p>
                ...
                <div class="dashboard-card" style="margin-top:1.5rem;">
                    <h3 class="mt0">Exigences couvertes ${Help.tip("Les exigences de référentiels reliées à cette mesure. La liaison se fait depuis le détail d'une exigence.")}</h3>
                    ${coverageHtml}
                </div>
