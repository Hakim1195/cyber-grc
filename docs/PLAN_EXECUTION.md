...
| **V3** | L3 authentification, L5 journal | 🟡 **OUVERTE le 03/09/2026** — cinq agents, périmètres disjoints au §3 ; six conditions d'entrée au `CONVENTIONS.md` §22 ; contrat de l'annuaire simulé au §25 | S3 |
...
| **Q-10** | ~160 ms d'analyse de corps avant toute décision, sans authentification. Ce n'est pas propre à la reprise : le remède est un contrôle en `onRequest`, avec la limitation de rythme | 🔵 mineur | **lot L3** | vague 3 | ouvert — **livrable ferme de la vague 3**, agent **A2** (condition d'entrée **E4**) |
| **Q-11** | Le repli d'`applyImport` réintroduit un renommage global quand le serveur ne porte pas `/api/reprise` : chemin atteignable seulement lors d'un retour arrière. À défaut de le fermer, le dire dans le code | 🔵 mineur | agent **FRONT** | avec Q-8 | ◐ **documenté, non fermé** → **`V1.1`** (§0 bis, 3ᵉ classe) — fermer exigerait une liste de champs tenue à la main, refusée au `CLAUDE.md` §3 (b) ; le bandeau compte désormais les réécritures hors de l'enregistrement renommé |
...
| **Q-28** | **`LONGUEUR_ALEA` est dupliquée en dur dans `src/reprise/index.ts`**, avec le rendu `toString(36).padStart(25, '0')` et le littéral `25`. Ce n'est **pas** une infraction à la règle « un seul générateur par langage » — c'est une **dérivation**, elle ne tire rien, et le §2 tient à la pureté de ce module. Mais si quelqu'un change la longueur d'un côté, les identifiants `-d-` et `-r-` **cessent de se ressembler**. Rien ne casse aujourd'hui ; signalé par son auteur plutôt que laissé sans propriétaire — c'est ce que le registre existe pour recevoir | 🔵 mineur | agent **API** | vague 3 | ouvert — report daté → **`V1.1`** : c'est une **dérivation**, elle ne tire aucun aléa ; une divergence produirait une allure, jamais une perte d'entropie (jugé ainsi par l'auditeur du 9ᵉ passage) |
...
| **Q-44** | **`LimitRequestBody` est inopérante sur `/api/`.** 28 311 552 octets traversent le frontal en entier alors que la borne est 27 262 976 — y compris posée dans un `<Location /api/>`. Contrôle symétrique dans le **même** serveur : le même envoi sur `/index.html` rend **413**. Le vhost et `install.sh` affirment l'inverse, et `install.sh` **imprime « ok » en comparant deux nombres dont l'un n'agit pas** — un contrôle qui se mesure lui-même. Contrôle **S13 en échec** | 🟠 majeur | agent **DÉPLOIEMENT** | immédiat | ◐ **borné au frontal pour le cas ordinaire** — le contournement `chunked` reste → ⚠️ **la justification ci-dessus était FAUSSE, et la mesure l'a dite.** E4 divise par ~3,3 le coût du *service* (291 → 90 ms pour 18 Mio anonymes derrière Apache) et **ne change rien au frontal** : jusqu'à **920 Mio** étaient encore relayés après E4. Ce qui ferme cette famille est la règle `Transfer-Encoding` d'**A5** — **411 en 63–169 ms, le service ne reçoit rien**. A2 écrit avoir *failli reconduire le report en recopiant sa justification* : c'est pourquoi cette ligne est **réécrite** et non reconduite (§22, règle de lecture). Reste non mesuré : les 18 Mio en `Content-Length` derrière le vhost d'après la règle, qui ne vise que `Transfer-Encoding` |
...
| **Q-60** | Le garde-fou de Q-52 ignore les **59 `<script src>`** d'`index.html` : un script publié mais non commité passerait | 🔵 mineur | agent **OUTILLAGE** | avant le 10ᵉ passage | ouvert — **vague 3**, agent **A4** : étendre le garde-fou aux 59 `<script src>` d'`index.html` |
| **Q-61** | `decisives()` compare **46 des 99 directives** du vhost — l'essai éprouve donc en partie sa propre réécriture | 🔵 mineur | agent **OUTILLAGE** | avant le 10ᵉ passage | ouvert — **vague 3**, agent **A4** : `decisives()` doit se découvrir dans le vhost, pas s'écrire à la main (§19.5) |
| **Q-62** | **Aucun essai ne joint les trois moitiés de S17.** L'auditeur l'a fait à la main, en une pièce, et c'était la première fois | 🔵 mineur | agent **OUTILLAGE** | vague 3 | ouvert — **vague 3**, agent **A4** : c'est le contrôle **S17** que la porte S3 rejouera, et il vaut mieux qu'il soit dans le banc que dans la main de l'auditeur |
...
| **Q-70** | **`administrationGroupe` est calculé DEUX FOIS, à l'identique** — `src/auth/index.ts` et `src/droits/resolveur.ts`. Signalé **par ses deux auteurs indépendamment** (A1 dans son code, A2 dans son rapport), ce qui est le meilleur indice qu'il faut le fermer : c'est le motif que ce chantier traque — deux rédactions de la même décision divergent, et la seconde n'est rejouée par personne. L'une doit **dériver** de l'autre | 🟠 majeur | agent **A1** | **avant la porte S3** | ouvert — le drapeau décide de l'accès Groupe ; une divergence y est une classe dure |
