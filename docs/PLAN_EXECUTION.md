...
| **Q-153** | **Un document de portée Groupe n'a pas de circuit de portée Groupe.** `approbations.filiale_id` est `not null` : la PSSI du groupe (`documents.filiale_id` nul) reçoit donc **un circuit par filiale**. C'est honnête — « qui a *adopté* cette politique ici » — mais ce n'est pas « qui a validé la politique du groupe », qui est la question du `PLAN_SERVEUR` §3.5 | 🟠 majeur | **orchestrateur** | `V1.1` | ouvert — le fermer demande une **migration** (`filiale_id` nullable, plus une politique d'écriture de portée Groupe) et un arbitrage : une politique groupe se valide-t-elle une fois au Groupe, ou filiale par filiale ? Les deux se défendent, et le client doit trancher |
