...
| **V3 · L5** | Journal d'audit — couverture, cloisonnement, consultation | 🟡 **construit, banc vert, porte S4 à jouer** — couverture portée de **4 actions émises sur 20** à **14**, mesurée en base et non lue dans le code ; condition **E6 fermée et vérifiée en vol** (`grc_lecture` lisait 160 entrées, il reçoit désormais « Périmètre non positionné ») ; trois routes de consultation, export CSV éprouvé sur une entrée hostile, vérification du chaînage. Banc **1136/1136**. Dix constats neufs (**Q-108 → Q-117**), dont **quatre trouvés en corrigeant** et deux — Q-108 et Q-116 — qui étaient des **essais verts ne mesurant rien** | **S4** |
...
| **Q-117** | **La machine porte la migration 008 et sert un binaire qui ignore les routes du journal.** L'agent qui a fermé E6 a appliqué `008_journal_lecture.sql` à la base de recette — acte de déploiement, annoncé dans son rapport. Mesuré après consolidation : `grc_lecture` reçoit bien « Périmètre non positionné » (E6 fermée **en vol**, ce que le dépôt seul ne prouvait pas), la connexion AD rend 200, et `GET /api/journal` rend **404** — le service exécute le build installé avant ce lot. C'est la situation exacte du constat **Q-103**, dans l'autre sens : ici le dépôt est en avance sur la machine, et rien ne fuit | 🔵 mineur | **orchestrateur** | **avant la porte S4** | ouvert — le redéploiement (`npm run build`, `rsync dist/`, `systemctl restart`, puis `install.sh --maj` pour le frontend) exige `sudo`, dont cette session ne dispose pas. À jouer par qui l'a, puis `install.sh --verifier-publication` |
