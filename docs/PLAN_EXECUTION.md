...
| **Q-181** | **Le profil DIRECTION livré avec le produit ne porte NI les risques NI les incidents.** Mesuré dans `profils` / `profil_domaines` : il ouvre `tableau_de_bord`, `synthese`, `echeances`, `exigences`, `mesures`, `referentiels` — tous en lecture, et rien d'autre. Conséquence directe sur l'écran **Vision Groupe**, qui lui est destiné : les colonnes risques et incidents affichent `—`, c'est-à-dire *« vous n'avez pas le droit de le savoir »* | 🟠 majeur | **utilisateur** | ouvert, à arbitrer | ⚠️ **Le produit se comporte correctement** — le contrat `null ≠ 0` fait exactement son travail, et c'est même la meilleure démonstration qu'il fonctionne. La question est **métier** : une direction de groupe qui pilote la cybersécurité veut-elle voir *combien de risques critiques porte chaque filiale* ? Si oui, ajouter `risques` et `incidents` au profil est un geste d'**administration**, pas un développement — le socle des profils est en base, modifiable. C'est écrit au `GUIDE_UTILISATEUR.md` §2 pour que la Direction ne s'en étonne pas |
...
| **Q-192** | **TREIZE codes du catalogue ANSSI sur quarante-deux désignent autre chose que ce que le guide officiel désigne sous le même numéro.** Mesuré contre l'édition **anglaise** du guide (*Guideline for a healthy information system in 42 measures*, v2, outil de suivi p. 60-65), croisée avec deux sources françaises : le code **30** est **inséré** dans la famille administration (le guide y range trois mesures, 27-29) ; les codes **31 → 41 sont décalés de +1** ; le code **42 FUSIONNE deux mesures distinctes** (analyse de risque et produits qualifiés) — et **c'est cette fusion qui ramène le total à 42 et masque le décalage** | 🟠 majeur | **utilisateur** | ouvert, à arbitrer | ⚠️ **Le défaut est dans le catalogue FRANÇAIS**, donc dans toutes les langues, et **les deux se lisent parfaitement** : un RSSI qui rapproche l'écran du guide officiel ne retrouve pas ses mesures, sans qu'aucune erreur ne le dise. **Rien n'a été renuméroté**, et le motif est décisif : les auto-évaluations sont stockées par `(ref_id, code)` — renuméroter **réattribuerait en silence des évaluations existantes à d'autres mesures**, dans un outil produit en audit. L'arbitrage appartient au produit : s'aligner sur le guide (avec migration des évaluations) ou **assumer un référentiel maison et le dire à l'écran**. La table de correspondance est en entête de `js/data/en/ref_anssi.js` |
...
| **Q-197** | **Analyseur XLSX quadratique** : 1 084 octets envoyés bloquent la boucle d'événements **8 362 ms**, ~2 Kio la bloquent **29 774 ms** — rapport 3,56× pour 2× d'entrée, mesuré **par la route**, sans `appliquer=oui` et sans limitation de rythme | 🟠 bloque le fonctionnement | **orchestrateur** | `V1.1` | ouvert — un déni de service à deux kilo-octets sur un produit accessible par VPN. Le correctif est algorithmique (l'analyseur recopie au lieu d'indexer) et n'appelle aucun changement de contrat |
| **Q-198** | **CSV : la borne des 5 000 lignes tombe APRÈS l'analyse.** 3,81 Mio font passer le tas de **144 à 1 071 Mio** (×243) **pour un fichier refusé**, contre `MemoryMax=2G` | 🟠 bloque le fonctionnement | **orchestrateur** | `V1.1` | ouvert — la borne protège la base, pas le serveur. Elle doit s'appliquer **pendant** l'analyse, pas après |
| **Q-199** | **L'unité de notification est livrée dans une configuration où elle ne PEUT PAS envoyer** — `IPAddressDeny=any` avec `IPAddressAllow=localhost` — et `install.sh` arme le minuteur sans le vérifier. Le seul contrôle d'allow-list interroge l'unité **applicative** | 🟠 bloque le fonctionnement | **orchestrateur** | `V1.1` | ouvert, et c'est **le pendant exact de Q-186** : le banc est vert sur la configuration qui empêche L12 de marcher. Le symptôme mentira sur sa cause — « Relais injoignable » alors que c'est le noyau qui refuse |
| **Q-200** | **`/api/consolidation` ne laisse aucune trace** — 0 entrée pour 5 lectures du groupe entier, témoin `/api/export` = 1 — **et l'essai verrouille l'absence** en citant une règle que `src/pieces/index.ts:745` contredit | 🔵 mineur | **orchestrateur** | `V1.1` | ouvert. ⚠️ **Cinquième occurrence** du motif « un essai qui MESURE un défaut et le CONSACRE comme une propriété désirable ». J'avais décidé de ne pas tracer ; l'auditeur montre que le dépôt fait l'inverse ailleurs, et que mon essai fige mon choix au lieu de l'éprouver |
...
| **Q-202** | **Un fichier RH déposé sur « Revues de direction » rend 200 et « 3 enregistrement(s) ont été créés » — trois revues VIDES dans une preuve d'audit** | 🟠 majeur | **orchestrateur** | `V1.1` | ouvert. L'import ne refuse pas un fichier dont **aucune** colonne ne correspond au modèle : il crée des lignes avec leurs seuls défauts. C'est la forme la plus discrète du premier cas du `CLAUDE.md` §3 — *quelque chose réussit en silence alors que c'est faux* |
| **Q-203** | **`conformite.js` : injection mesurée SOUS LA CSP RÉELLE** — l'élément entre dans le DOM, `position:fixed` s'applique, seul le gestionnaire est bloqué. L'internationalisation a remplacé une table fermée par un passe-plat, et deux `innerHTML` ne l'échappent pas | 🟠 majeur | **orchestrateur** | `V1.1` | ouvert — la CSP arrête l'exécution, pas l'habillage : un cadre superposé suffit à tromper. C'est le pendant de **Q-183** : l'i18n révèle des injections, et elle en introduit |
| **Q-204** | **La barrière anti-balise des traductions ne couvre pas les CATALOGUES**, que des agents écrivaient au moment de l'audit | 🔵 mineur | **orchestrateur** | `V1.1` | ouvert — `test/depot/traductions.test.mjs` refuse `<` et `>` dans les valeurs de `js/i18n/`, et rien n'équivaut pour `js/data/en/`. Le contrôle existe, il ne regarde pas au bon endroit |
