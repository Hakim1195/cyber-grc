...
| **Q-13** | **Le générateur d'identifiants de la reprise n'a que MILLE valeurs d'aléa — c'est le défaut bloquant du 3ᵉ passage, vivant dans un second fichier.** `src/reprise/index.ts:513` rend `<PRÉFIXE>-<ms>-<0..999>`, et sert à deux chemins qui écrivent en base : les enregistrements du fichier sans identifiant, et **tous les `MMO-` engendrés par `scinderMesures`**, dans une boucle serrée où l'horodatage ne sépare plus rien. Mesuré (200 essais) : **42 mesures → 1,8 % de collisions, 250 → 11,1 %, 1000 → 32,8 %**. Le commentaire de la fonction invoque l'alignement sur `UI.genId` et `f_generer_id` — les deux ont été durcis, lui non : **dixième occurrence du motif**. Gravité à confirmer par le comportement en aval (perte silencieuse ou échec bruyant) | 🔴 **bloquant présumé** | agent **API**, périmètre élargi à `src/reprise/**` | **avant la fermeture de la porte S2** |
