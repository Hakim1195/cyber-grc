...
| **Q-72** | **Le compte de secours est inutilisable dans la situation exacte pour laquelle il existe.** Le câblage de `src/serveur.ts` monte l'authentification réelle si `ldapActif && ldap !== null` — il **ignore le compte de secours**. Un serveur dont le compte de service AD est verrouillé, cas nommé au `.env.example`, retombe donc sur la session provisoire, qui refuse tout. Rien ne fuit (fail-closed) ; le secours ne secourt rien | 🟠 majeur | **orchestrateur** (le câblage) · agent **AUTH** (la décision) | **avant la mise en service pilote** | ouvert — trouvé en réparant T-3, pas par un essai |
