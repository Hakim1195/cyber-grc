...
| **La recette complète** | ✅ en ligne en permanence | `systemctl is-active cyber-grc apache2 postgresql` ; `https://grc-test.site/` → 200 |
...
| **L17 — Prise en main** | ✅ **LIVRÉ EN ENTIER le 21/09/2026** — recherche globale et palette `Ctrl+K` (A3), menu replié en sept sections (A2), **écran de démarrage par rôle (A4)** et **Kanban du plan d'actions (A5)**. ⚠️ **A4 et A5 étaient restés dus ONZE JOURS** pendant que quatre vagues postérieures se fermaient : la vague V-A n'avait aucune ligne de clôture, et personne ne l'avait remarqué. ⚠️ **« Par rôle » se DÉRIVE des droits** — le produit porte neuf profils de socle et un client peut en composer d'autres ; quatre écrans écrits pour quatre noms auraient montré à un auditeur ce qu'on destinait à un contributeur. ⚠️ Et le Kanban porte **deux boutons de déplacement par carte** : *une fonctionnalité qui n'existe qu'à la souris est absente pour une partie des utilisateurs* |
...
| **L20 — Réglementaire opérationnel** | ✅ **LIVRÉ EN ENTIER le 21/09/2026** — horloge NIS2/RGPD (`034`), AIPD (`039`), demandes d'exercice de droits (`040`), main courante de crise en ajout seul (`041`), **et 20.2 : les formulaires ANSSI et CNIL** pré-remplis depuis la fiche d'incident, pour les deux régimes (NIS2 art. 23, RGPD art. 33). ⚠️ **Le produit NE TRANSMET RIEN à une autorité** — il prépare, l'humain envoie, et le document le porte en toutes lettres ; le bouton dit « préparer », jamais « déclarer ». ⚠️ **Et le formulaire DIT SES MANQUES** : *un formulaire à moitié rempli est plus dangereux qu'un formulaire vide — vide, on le remplit ; à moitié rempli, on l'envoie.* Chaque rubrique absente est nommée, avec ce que le texte attend |
