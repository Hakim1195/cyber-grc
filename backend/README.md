...
npm test                           # 2189 essais, trente et une familles (voir plus bas)
...
| | |
|---|---|
| Révision mesurée | **`28c5438`** — lot **L25**, EBIOS RM en entier (ateliers 1 à 5), 19/09/2026, relevée **sur la machine réelle** (Debian 13, `SRV-Infra`). ⚠️ **Ce bloc a déjà été RÉANCRÉ six fois**, dont une où il désignait une révision cinquante-six commits en arrière (constat **Q-219**). Le garde-fou juge le document contre **la révision que le document nomme**, jamais contre l'arbre de travail. |
| État de l'arbre | **arbre de `28c5438`** plus la documentation du commit qui suit. Compte **RELEVÉ famille par famille**, jamais déduit — une estimation en avait annoncé 2 051 pour 2 048 réels. Quatre familles bougent depuis `941ea8b`, dont une NEUVE : `ebios` **15**, `depot` 57 → **60**, `modules` 46 → **47**, `documentation` 32 → **33**. ⚠️ `documentation` est relevée **verte**, et il faut dire pourquoi : à la mesure, ce bloc nommait encore la révision précédente, que le contrôle jugeait donc légitimement. Seule circularité du dispositif ; elle se dit plutôt qu'elle ne se cache, et c'est le rejeu d'APRÈS qui vaut. |
...
npm test                                         → tests 2189 · pass 2189 · fail 0
                                                   ...
                                                   journal 19 · ebios 15 · recherche 13
