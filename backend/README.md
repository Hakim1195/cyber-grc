...
npm test                           # 2031 essais, vingt-cinq familles (voir plus bas)
...
**Mesuré au 16/09/2026, à la révision `396cd86`** : `npm test` → **2031 essais,
2031 passés** ; `verifier-types` propre ; `npm audit --omit=dev` → 0 vulnérabilité ;
...
| État de l'arbre | **arbre PROPRE de `396cd86`**, `git status` vide. Compte relevé **famille par famille**, chaque répertoire joué séparément. ⚠️ **`documentation` est relevée à 30 essais dont UN EN ÉCHEC** — celui qui juge ce bloc-ci, et qui ne peut pas être vert avant que le bloc soit écrit. C'est la seule circularité du dispositif, et elle se dit plutôt qu'elle ne se cache : la famille est rejouée APRÈS cette mise à jour. Deux familles bougent depuis `8e0b149`, et ce sont les deux moitiés de 19.4 : `pieces` 125 → **141**, `navigateur` 203 → **207**. |
...
npm test                                         → tests 2031 · pass 2031 · fail 0
                                                   base 311 · api 297 · navigateur 207
