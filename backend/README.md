...
cotation, versionnées et datées) : `npm test` → **2231 essais, 2231 passés** ; `verifier-types` propre ; `npm audit --omit=dev` → 0 vulnérabilité ;
...
| | |
|---|---|
| Révision mesurée | **`3ae666c`** — les échelles de cotation (action 25.3) et les trois défauts que le NAVIGATEUR a trouvés après 2 230 essais verts, 19/09/2026, relevée **sur la machine réelle** (Debian 13, `SRV-Infra`). ⚠️ **Ce bloc a déjà été RÉANCRÉ sept fois**, dont une où il désignait une révision cinquante-six commits en arrière (constat **Q-219**). Le garde-fou juge le document contre **la révision que le document nomme**, jamais contre l'arbre de travail. |
| État de l'arbre | **arbre de `3ae666c`** plus la documentation du commit qui suit. Compte **RELEVÉ famille par famille**, jamais déduit — chaque répertoire de `test/` joué séparément. Quatre familles bougent depuis `a0cb1d7`, dont une NEUVE : `echelles` **23**, `api` 297 → **299**, `navigateur` 237 → **238**, `modules` 50 → **51**. ⚠️ `documentation` est relevée **verte**, et il faut dire pourquoi : à la mesure, ce bloc nommait encore la révision précédente, que le contrôle jugeait donc légitimement. Seule circularité du dispositif ; elle se dit plutôt qu'elle ne se cache, et c'est le rejeu d'APRÈS qui vaut. |
