...
→ 0 anomalie, **19 garde-fous consignés**, **21 migrations**, **50 tables**.
...
select * from f_verifier_schema()                → 0 ligne (19 garde-fous découverts, joués, consignés)
```

Schéma relevé **dans le catalogue**, pas dans le texte des migrations : **50 tables** en
**21 migrations**, **200 politiques**, **0 table sans RLS activée, 0 sans RLS forcée**,
...
`(id, filiale_id)`, **9 unicités** `uq_<parent>_id_filiale`, **19 contrôles consignés**
...
- **50 tables**, obtenues aujourd'hui en **21 migrations** appliquées de bout en bout par
