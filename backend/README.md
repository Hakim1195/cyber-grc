...
| `test/ouverture/` | les **jetons d'API et les événements sortants** (L22). Un jeton est un **SUJET DE DROITS** : il bâtit un `EtatSession` et traverse la RLS comme un compte, et son niveau se **rabat sur le plus faible des domaines demandés** — jamais sur le plus fort, sinon un jeton « lecture seule » sur un domaine où le porteur écrit rendrait le droit d'écrire. 🛑 Le §6 mesure ce que **l'APPELANT HTTP reçoit**, et non ce que la fonction rend : c'est ce qui manquait aux dix-huit essais qui laissaient passer un jeton rendant **401 à son premier usage** (`CONVENTIONS.md` §46). ⚠️ Et un essai éprouve que ce qui est **ADMIS** au vocabulaire des abonnements est bien **ÉMIS** par quelqu'un — un événement que personne n'émet est une promesse faite à un abonné |
...
`f_verifier_schema()` → 0 anomalie, **64 garde-fous consignés**, **60 migrations**,
**91 tables**, **528 décisions** au registre de l'article 30 ; publication → **88
...
⚠️ **La deuxième réserve est propre à CETTE machine, et le chiffre annoncé ici était faux
depuis trois jours** : `profil DÉCOUVERTE — cette installation n'est pas de production`.
La recette est en profil découverte **depuis le 15/09/2026** — `/etc/cyber-grc/env.avant-profil-decouverte`
porte cette date —, si bien que « 14 conformes, 1 réserve » ne décrivait plus rien au moment
où il a été écrit. Une installation de production n'a que la réserve `SMTP_ACTIF=non`.
⚠️ **Aucun garde-fou ne regarde ce bilan** : il exige `sudo` et l'état d'une machine, ce
qu'un banc ne peut pas atteindre. Il se relève donc à la main, et il vieillit — c'est un
constat de la famille **Q-219**, dit plutôt que caché.
...
| **L17 — Prise en main** | 🟡 **partiellement livré** les 14 et 16/09/2026 : **menu regroupé** (32 entrées à plat → sept sections repliables), **recherche globale et palette `Ctrl+K`** (bornée par la RLS côté serveur, jamais par un filtre côté client), architecture des sections et passe de style. ⚠️ **Reste A4** (écran de démarrage par rôle) **et A5** (Kanban du plan d'actions) — voir `../docs/REPRISE.md` §6. ⚠️ Et la contrainte « pas avant S7 et S8 » est levée par l'**arbitrage utilisateur du 14/09/2026** : les passages de porte sont arrêtés, l'`ultrareview` vient à la fin |
| **L18 — Installation en une commande** | ✅ **livré — 18.7 excepté**. `install.sh --assistant` pose les **six** valeurs que le script ne peut pas deviner et enchaîne l'installation ; `--diagnostic` rend l'état de **quinze sujets** sans rien modifier, en code 0/1/2 — douze à l'origine, quatorze depuis le constat Q-290 (la quarantaine et le minuteur de ré-analyse), **quinze depuis L27** (le mode de l'assistance IA). ⚠️ *Ce paragraphe a annoncé « quatorze » et « 13 conformes » le 19/09/2026, le jour même où L27 posait le quinzième : le chiffre avait été RECOPIÉ au lieu d'être rejoué. C'est le motif du constat Q-290, refait sur la ligne qui le raconte.* **Profil découverte** : ni annuaire ni courriel, compte de secours dont l'empreinte `scrypt` est calculée par `dist/auth/secours.js` — jamais recopiée en shell —, certificat auto-signé. `../docs/INSTALLER.md` : cinq commandes, aucun renvoi. ⚠️ Les six premiers sous-lots ne touchent **ni `src/` ni le schéma** : c'est ce qui les a autorisés à se jouer avant les portes. **Le bandeau 18.2 b est livré le 09/09/2026** : `installation.profil` dans la **charte de session** — donc dans `GET /api/session` **et** `POST /api/connexion`, identiques à l'octet près (§26.2) —, bandeau **sans bouton de fermeture** et **imprimé avec les fiches**, reposé à chaque écran. ⚠️ **Une valeur inconnue de `CYBER_GRC_PROFIL` refuse le démarrage** ; une valeur **absente** vaut « production » (tout le parc antérieur à L18). ⚠️ **Il touche `src/` et la SPA : à déclarer au 7ᵉ passage de S8.** Reste **18.7** (assistant de premier démarrage), après les portes |
| **L18 bis — Jeu de découverte** | ✅ **livré le 14/09/2026** (migration `032`) — un groupe industriel fictif complet, et les **cinq conditions constitutives** tenues : marque d'origine **dans la donnée** et non seulement à l'écran (inforgeable), geste volontaire, **refus s'il existe la moindre ligne réelle**, purge complète par le déclencheur `017`, interdit hors profil découverte. Le brief interdisait les données de démonstration ; l'utilisateur a levé l'interdit sous ces cinq conditions |
| **L19 — Chaîne de preuve** | ✅ **livré EN ENTIER** les 15 et 16/09/2026 (migrations `033` à `038`) — attestation de lecture, dérogations datées dont l'état se **dérive**, lien document ↔ contrôle, contrôle périodique et efficacité **distincte de la maturité**, et **réutilisation d'une preuve** (une procédure déposée une fois prouve cinq contrôles ; le fichier ne part qu'au **dernier** détachement). ⚠️ Chacune porte son écran ET son garde-fou : *une capacité qu'aucun écran n'appelle est une capacité absente* |
...
| **L22 — Ouverture technique** | ✅ **LIVRÉ le 19/09/2026** — jetons d'API (`053`), événements sortants, cadre de connecteurs (`054`) et vocabulaire clos des réglages (`055`). ⚠️ **Un jeton est un SUJET DE DROITS** : il bâtit un `EtatSession` et traverse la RLS, il ne la contourne pas ; son niveau se rabat sur **le plus faible des domaines demandés**, et non sur le plus fort. ⚠️ **Le service n'appelle JAMAIS vers l'extérieur** : les déclencheurs ENFILENT, une unité systemd distincte draine — même discipline que L12 et que le journal vers rsyslog. ⚠️ **L'action 22.6 (Jira / ServiceNow) est la COMBINAISON de 22.1 et 22.3** : l'aller par l'événement, le retour par un jeton sur la route générique — ni client Jira, ni client ServiceNow, donc rien à maintenir |
| **L23 — Collecte automatique** | ✅ **LIVRÉ le 19/09/2026** (`054`, `056`) — trois connecteurs locaux (sauvegarde, antivirus, annuaire), le constat DATÉ rattaché à une mesure, l'historique du contrôle, et l'indicateur « preuves fraîches » dans les courbes existantes. ⚠️ **AUCUN chemin d'échec ne rend « conforme »** (critère 23.4) : c'est mesuré sur le REGISTRE des exécuteurs, pas sur une liste de trois. ⚠️ **Une preuve périmée redevient absente**, et « périmé » n'est pas « non conforme ». ⚠️ Le **PASSAGE** au rouge ouvre une action ; rester rouge n'en ouvre pas — sans quoi un contrôle qui passe toutes les cinq minutes créerait 288 actions par jour |
| **L27 — Assistance IA** | ✅ **CONSTRUIT le 19/09/2026** (migration `057`, `src/assistance/`, écran en onglet des Paramètres) — les **cinq usages** arbitrés, le modèle **local par défaut**, les **six barrières** du mode externe. ⚠️ **L'IA PROPOSE, UN HUMAIN DÉCIDE** : aucun usage n'écrit en base, et un essai le MESURE sur les quatre tables métier. ⚠️ **Le mode local ne sort pas, et c'est mesuré sur la machine réelle** : `IPAddressDeny=::/0 0.0.0.0/0`, seule la boucle locale autorisée, et l'assistance répond. ⚠️ **Le troisième verdict est le cœur du lot** — source coupée, réponse illisible, modèle absent rendent « indisponible » : *jamais une réponse inventée, jamais un silence*. ⚠️ **Le mode externe est FERMÉ** : sans `CYBER_GRC_IA_EXTERNE`, aucune ligne d'activation n'entre en base, quelle que soit la route |
| **L28 — Portail fournisseur** | ✅ **CONSTRUIT le 19/09/2026** (migration `058`, `src/portail/`, vhost dédié) — lien signé sans compte, session d'**un seul objet**, dépôt par **LA** chaîne du lot L6, reprise datée, attestation, trois actions de journal. 🛑 **CONSTRUIT N'EST PAS OUVERT** : `PORTAIL_ACTIF=non` par défaut **n'enregistre aucune route** — pas une qui rendrait 503, pas un montage à moitié —, et le vhost est livré **désactivé**. ⚠️ **La consigne du plan prime** : la porte **S15** est la plus exigeante du chantier, et *en cas de doute sur ce lot, on ne livre pas*. Il ne s'ouvre pas avant l'ultrareview |
...
select * from f_verifier_schema()                → 0 ligne (64 garde-fous découverts, joués, consignés)
```

Schéma relevé **dans le catalogue**, pas dans le texte des migrations : **91 tables** en
**60 migrations**, **364 politiques**, **0 table sans RLS activée, 0 sans RLS forcée**,
**176 clés étrangères** (104 `restrict`, 62 `cascade`, 9 `set null`, 1 `no action`),
**83 tables portant `cree_par` et 81 déclencheurs de création**, **58 clés étrangères
composites** visant
`(id, filiale_id)`, **28 unicités** `uq_<parent>_id_filiale`, **64 contrôles consignés**
...
- **91 tables**, obtenues aujourd'hui en **60 migrations** appliquées de bout en bout par
...
- **364 politiques**, RLS **activée et forcée** sur **toutes** les tables, propriétaire
  compris : mesuré dans `pg_class`, **0 table sans `relrowsecurity`, 0 sans
  `relforcerowsecurity`**.
- **176 clés étrangères**, relevées dans `pg_constraint` et non dans le texte des
  migrations : **104 en `restrict`, 62 en `cascade`, 9 en `set null`**. ⚠️ **Elles sont
...
- **Clés étrangères et unicités composites** : quand l'enfant et le parent sont tous
  ...
  `uq_<parent>_id_filiale`. Relevé dans `pg_constraint` : **58 clés étrangères** dont la
