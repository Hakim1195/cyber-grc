...
**État : lot L0 livré ; lot L1 livré, revue de sécurité S1 en cours d'instruction.**
Voir « Avancement » en fin de document.
...
Le script monte deux filiales de démonstration, puis rend un tableau de **28
contrôles** avec pour chacun l'attendu, l'obtenu et son verdict : lecture, écriture,
liaisons, mesures, périmètre, journal, privilèges du rôle applicatif, couverture de
la RLS. Si un seul contrôle échoue, il lève une exception et sort en erreur — un
...
npm test                           # 144 tests (socle, RLS, reprise grc-backup)
...
État réel des lots. La **conduite** du chantier — découpage en vagues, portes de
sécurité, définition de « terminé » — vit dans
...
| **L1 — Schéma relationnel** | ✅ **livré** — porte de sécurité **S1 en cours d'instruction**, aucun verdict à ce jour |
...
- **Schéma relationnel — 47 tables**, appliquées de bout en bout sur base neuve par
  ...
  - `004_rls.sql` — privilèges, **188 politiques**, RLS **activée et forcée sur les
    47 tables**, déclencheurs de cohérence et garde-fou de couverture.
- **Cascades du `CONVENTIONS.md` §8 portées par le schéma** (vérifié sur la base :
  `actions` en `cascade` depuis exigence, risque, évaluation et incident, en
  `set null` depuis `mesure_catalogue` ; `tests_pra` en `cascade` depuis son
  scénario ; `incidents.risque_id` en `set null` ; dépendances d'actifs purgées dans
  les deux sens).
- **Cloisonnement démontré** : `db/verifier_cloisonnement.sql` joué avec `grc_app`,
  **28 contrôles réussis, 0 échec**.
...
- **Banc d'essai** : **144 tests** `node:test` (78 sur la base — socle, journal,
  verrouillage optimiste, RLS, privilèges ; 66 sur la reprise), 0 échec.
- **Reprise des exports `grc-backup`** (`src/reprise/**`) : portage serveur des
  ...
  exact des identifiants. Module pur : il n'écrit ni en base, ni sur le disque.
- **Outillage** : `db/migrate.mjs` (ordre déterministe, empreinte SHA-256, compte
  propriétaire imposé, codes de sortie documentés) et `db/dev/preparer_base_dev.sh`.
- **Déploiement** : unité systemd durcie, vhost Apache, durcissement de portée
  serveur, `install.sh` idempotent qui engendre les secrets internes et **vérifie
  que la base n'appartient pas au compte du service**.

### Réserves — ce qui n'est pas vérifié

- La **porte de sécurité S1** (`../docs/PLAN_EXECUTION.md` §4) est en cours
  d'instruction par un auditeur indépendant. Le lot L1 est livré ; il n'est **pas**
  validé, et aucun verdict n'est annoncé ici.
- Les vérifications ci-dessus ont été menées sur **PostgreSQL 16**, alors que la
  cible du déploiement est **PostgreSQL 17** (dépôt PGDG, `install.sh`).
