...
describe('Le dernier chemin non éprouvé : un vieil export réel, de bout en bout', () => {
  test('un export v6 volumineux traverse VINGT-QUATRE paliers ET atterrit en base', async () => {
    ...
    assert.equal(reponse.corps.rapport.version_cible, 30);
    assert.equal(reponse.corps.rapport.paliers.length, 24, 'v6 → v30, c’est vingt-quatre paliers.');
  ...
  test('250 mesures SANS identifiant : le serveur en engendre 250 DISTINCTS', async () => {
    ...
    assert.equal(
      ...
      'Aucun doublon ne doit être reproché à un fichier qui n’en porte pas : '
        + JSON.stringify(reponse.corps).slice(0, 1200),
    );
