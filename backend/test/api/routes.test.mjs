...
describe('Les sept points d’entrée du lot L2 répondent', () => {
  ...
  test('GET /api/modele — décrit les 41 entités, et ne fuit aucun nom de table', async () => {
    const { statut, corps } = await serveur.appeler('GET', '/api/modele');
    assert.equal(statut, 200);
    assert.equal(Object.keys(corps.entites).length, 41);
    assert.equal(corps.schemaVersion, 24);
  ...
  test('GET /api/donnees — rend le jeu de la filiale, dans la forme de « data »', async () => {
    ...
    assert.equal(statut, 200);
    assert.equal(corps.data.schemaVersion, 24);
