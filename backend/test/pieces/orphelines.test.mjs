...
async function vocabulairesDuSchema() {
  return await base.avecPerimetre(
    applicatif,
    perimetre('temoin', FILIALE_A, [FILIALE_A]),
...
async function creer(entite, champs = {}) {
  ...
  vocabulaires ??= await vocabulairesDuSchema();
...
describe('Q-232 — la CASCADE emporte aussi les pièces des enfants', () => {
  ...
  test('CHAQUE cascade : la pièce de l’enfant suit — ligne, fichier, délivrance', async () => {
    ...
    for (const { parent, enfant, colonne } of cascades) {
      ...
      // ⚠️ **Un enfant peut nommer le même parent par PLUSIEURS colonnes** —
      // `prestataire_sous_traitance` le fait par le donneur d'ordre ET le
      // sous-traitant, toutes deux `not null`. Les autres reçoivent chacune un
      // parent DISTINCT : la contrainte de boucle refuse qu'un tiers se
      // sous-traite à lui-même, et réutiliser `idParent` ferait échouer l'essai
      // pour une raison étrangère à ce qu'il mesure.
