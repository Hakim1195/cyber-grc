...
test('un export À LA VERSION COURANTE traverse la chaîne sans être modifié', () => {
  ...
  const origine = instantaneV18Complet();
  const resultat = reprendreExport(fichier(18, origine), OPTIONS_FIGEES);
...
test('un export à la version courante ne produit aucun avertissement', () => {
  const resultat = reprendreExport(fichier(18, instantaneV18Complet()), OPTIONS_FIGEES);
...
test('aller-retour complet : reprise → enveloppe → reprise rend le même instantané', () => {
  ...
  const premier = reprendreExport(fichier(18, origine), OPTIONS_FIGEES);
  ...
  assert.equal(renvoi.version, 18);
...
test('les volumes du rapport recensent les 26 collections', () => {
  const resultat = reprendreExport(fichier(12, instantaneV12Complet()), OPTIONS_FIGEES);
