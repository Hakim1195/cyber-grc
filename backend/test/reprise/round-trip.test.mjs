...
test('un export À LA VERSION COURANTE traverse la chaîne sans être modifié', () => {
  ...
  const origine = instantaneV14Complet();
  const resultat = reprendreExport(fichier(14, origine), OPTIONS_FIGEES);
...
test('un export à la version courante ne produit aucun avertissement', () => {
  const resultat = reprendreExport(fichier(14, instantaneV14Complet()), OPTIONS_FIGEES);
...
test('aller-retour complet : reprise → enveloppe → reprise rend le même instantané', () => {
  ...
  const premier = reprendreExport(fichier(14, origine), OPTIONS_FIGEES);
  ...
  assert.equal(renvoi.version, 14);
