...
test('un export À LA VERSION COURANTE traverse la chaîne sans être modifié', () => {
  ...
  const origine = instantaneV17Complet();
  const resultat = reprendreExport(fichier(17, origine), OPTIONS_FIGEES);
...
test('un export à la version courante ne produit aucun avertissement', () => {
  const resultat = reprendreExport(fichier(17, instantaneV17Complet()), OPTIONS_FIGEES);
...
test('aller-retour complet : reprise → enveloppe → reprise rend le même instantané', () => {
  ...
  const premier = reprendreExport(fichier(17, origine), OPTIONS_FIGEES);
  ...
  assert.equal(renvoi.version, 17);
...
test('les volumes du rapport recensent les 25 collections', () => {
  const resultat = reprendreExport(fichier(12, instantaneV12Complet()), OPTIONS_FIGEES);
