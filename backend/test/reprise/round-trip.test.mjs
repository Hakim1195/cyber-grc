...
import {
  ...
  instantaneV23Complet,
  OPTIONS_FIGEES,
} from './jeux-essai.mjs';

test('un export À LA VERSION COURANTE traverse la chaîne sans être modifié', () => {
  // ⚠️ Cet essai employait un instantané v12, et il est devenu FAUX le jour où
  // une collection a été ajoutée : un v12 traverse désormais un palier, à juste
  ...
  const origine = instantaneV23Complet();
  const resultat = reprendreExport(fichier(23, origine), OPTIONS_FIGEES);
...
test('un export à la version courante ne produit aucun avertissement', () => {
  const resultat = reprendreExport(fichier(23, instantaneV23Complet()), OPTIONS_FIGEES);
...
test('aller-retour complet : reprise → enveloppe → reprise rend le même instantané', () => {
  ...
  const premier = reprendreExport(fichier(23, origine), OPTIONS_FIGEES);
  ...
  assert.equal(renvoi.version, 23);
...
test('les volumes du rapport recensent les 29 collections', () => {
  ...
  // ⚠️ **26 est un compte d'ENREGISTREMENTS, pas de collections** — et les deux
  // valaient 26 jusqu'à la v19, ce qui rendait la confusion invisible. Le fichier
  // d'essai est une v12 : le nombre de ses lignes ne bouge pas quand une collection
  // s'ajoute au modèle. *Deux grandeurs qui coïncident un temps finissent par
  // diverger, et c'est ce jour-là qu'on découvre laquelle on mesurait.*
