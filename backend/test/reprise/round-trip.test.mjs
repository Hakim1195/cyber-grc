...
test('un export À LA VERSION COURANTE traverse la chaîne sans être modifié', () => {
  ...
  const origine = instantaneV26Complet();
  const resultat = reprendreExport(fichier(26, origine), OPTIONS_FIGEES);
...
test('un export à la version courante ne produit aucun avertissement', () => {
  const resultat = reprendreExport(fichier(26, instantaneV26Complet()), OPTIONS_FIGEES);
...
test('aller-retour complet : reprise → enveloppe → reprise rend le même instantané', () => {
  ...
  const premier = reprendreExport(fichier(26, origine), OPTIONS_FIGEES);
  ...
  assert.equal(renvoi.version, 26);
