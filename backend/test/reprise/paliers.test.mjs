...
test('v1 → v2 : audits et revues rejoignent l’instantané unifié', () => {
  ...
  // Une v1 traverse les SEIZE paliers — le quatorzième (v14 → v15) ajoute le champ
  ...
  // « analyses_impact » (20.3).
  assert.equal(rapport.paliers.length, 17);
  assert.equal(rapport.versionOrigine, 1);
  assert.equal(rapport.versionCible, 18);
});
...
test('v2 → v3 : évaluations de référentiels et pivot « Mesure de sécurité »', () => {
  ...
  assert.equal(rapport.paliers.length, 16);
});
...
test('reprise d’un bout en bout : une v1 arrive en v18 avec ses 26 collections', () => {
  const { rapport, charge } = reprendre(1);

  assert.equal(charge.schemaVersion, 18);
  assert.equal(rapport.paliers.length, 17);
  assert.equal(Object.keys(rapport.volumes).length, 26);
  // Les dix-sept paliers se suivent sans trou : 1→2, 2→3, … 17→18.
...
test('un fichier qui ment sur sa version est rattrapé, et le mensonge est signalé', () => {
  ...
  const resultat = reprendreExport(fichier(18, charge), OPTIONS_FIGEES);

  assert.equal(resultat.statut, 'reprise');
  assert.equal(resultat.rapport.paliers.length, 0, 'une v18 déclarée ne traverse aucun palier');
