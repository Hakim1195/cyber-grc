...
test('v1 → v2 : audits et revues rejoignent l’instantané unifié', () => {
  ...
  // Une v1 traverse les QUATORZE paliers — le treizième (v13 → v14) ajoute les
  // dérogations datées (action 19.2), le quatorzième (v14 → v15) le champ
  // « mesures_ids » des documents (action 19.3).
  assert.equal(rapport.paliers.length, 14);
  assert.equal(rapport.versionOrigine, 1);
  assert.equal(rapport.versionCible, 15);
});
...
test('v2 → v3 : évaluations de référentiels et pivot « Mesure de sécurité »', () => {
  ...
  assert.equal(rapport.paliers.length, 13);
});
...
test('reprise d’un bout en bout : une v1 arrive en v15 avec ses 24 collections', () => {
  const { rapport, charge } = reprendre(1);

  assert.equal(charge.schemaVersion, 15);
  assert.equal(rapport.paliers.length, 14);
  assert.equal(Object.keys(rapport.volumes).length, 24);
  // Les quatorze paliers se suivent sans trou : 1→2, 2→3, … 14→15.
...
test('un fichier qui ment sur sa version est rattrapé, et le mensonge est signalé', () => {
  ...
  const resultat = reprendreExport(fichier(15, charge), OPTIONS_FIGEES);

  assert.equal(resultat.statut, 'reprise');
  assert.equal(resultat.rapport.paliers.length, 0, 'une v15 déclarée ne traverse aucun palier');
