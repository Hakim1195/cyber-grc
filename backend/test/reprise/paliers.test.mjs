...
test('v1 → v2 : audits et revues rejoignent l’instantané unifié', () => {
  ...
  // Une v1 traverse les QUINZE paliers — le quatorzième (v14 → v15) ajoute le champ
  // « mesures_ids » des documents (19.3), le quinzième (v15 → v16) l'efficacité et le
  // rythme de rejeu des contrôles (19.5 / 19.6).
  assert.equal(rapport.paliers.length, 15);
  assert.equal(rapport.versionOrigine, 1);
  assert.equal(rapport.versionCible, 16);
});
...
test('v2 → v3 : évaluations de référentiels et pivot « Mesure de sécurité »', () => {
  ...
  assert.equal(rapport.paliers.length, 14);
});
...
test('reprise d’un bout en bout : une v1 arrive en v16 avec ses 24 collections', () => {
  const { rapport, charge } = reprendre(1);

  assert.equal(charge.schemaVersion, 16);
  assert.equal(rapport.paliers.length, 15);
  assert.equal(Object.keys(rapport.volumes).length, 24);
  // Les quinze paliers se suivent sans trou : 1→2, 2→3, … 15→16.
...
test('un fichier qui ment sur sa version est rattrapé, et le mensonge est signalé', () => {
  ...
  const resultat = reprendreExport(fichier(16, charge), OPTIONS_FIGEES);

  assert.equal(resultat.statut, 'reprise');
  assert.equal(resultat.rapport.paliers.length, 0, 'une v16 déclarée ne traverse aucun palier');
