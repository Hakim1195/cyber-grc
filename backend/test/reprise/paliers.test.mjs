...
test('v1 → v2 : audits et revues rejoignent l’instantané unifié', () => {
  ...
  assert.equal(rapport.versionCible, 17);
});
...
test('v2 → v3 : évaluations de référentiels et pivot « Mesure de sécurité »', () => {
  ...
  assert.equal(rapport.paliers.length, 15);
});
...
test('reprise d’un bout en bout : une v1 arrive en v17 avec ses 25 collections', () => {
  const { rapport, charge } = reprendre(1);

  assert.equal(charge.schemaVersion, 17);
  assert.equal(rapport.paliers.length, 16);
  assert.equal(Object.keys(rapport.volumes).length, 25);
  // Les seize paliers se suivent sans trou : 1→2, 2→3, … 16→17.
...
test('un fichier qui ment sur sa version est rattrapé, et le mensonge est signalé', () => {
  ...
  const resultat = reprendreExport(fichier(17, charge), OPTIONS_FIGEES);

  assert.equal(resultat.statut, 'reprise');
  assert.equal(resultat.rapport.paliers.length, 0, 'une v17 déclarée ne traverse aucun palier');
